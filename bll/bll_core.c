...
/** watch dog **/
static int watch_listener_process(void *p, int msg, void *arg)
{
    //PrivInfo *thiz = ((vmp_node_t *)p)->private;
    PrivInfo *thiz = p;

    VMP_LOGD("ipc_watchdog_feed");
    ipc_watchdog_feed(&thiz->ipcs);
    VMP_LOGD("ipc_watchdog_feed end");
...
static int task_server_listener(PrivInfo *thiz)
{
    ...
    req.port        = 9999;
    req.ctx         = thiz;
    req.pfn_proc    = watch_listener_process;
